...
*                                                                         *
*    DATE CREATED:         09-17-92                                       *
*                                                                         *
*    DATE LAST MODIFIED:   09-08-93                                       *
*                                                                         *
*    VERSION:              1.00                                           *
*                                                                         *
*    This program displays a phase plane plot on the screen in real time  *
*    for a damped, driven pendulum.  The equations of motion are          *
...
/* Function Prototypes are in pendsim.h                                    */
...
{
	int         which, old_sav_flag, npts = 0, save_flag = FALSE, dummy;
	int         show_graphics = YES, cnt = 0, dum_char;
	char        txt[60];
	double      t, dt;
	double      th_new, om_new, th_old, om_old;
	double      q, g, wd;
	double      x, y;
	static char resp_nam[] = {"\\pendresp.dat"};
	...
	if(( pendresp = fopen( direc, "w" ) ) != NULL )
	{
		...
		_settextcolor( CYAN );
		sprintf( txt, "Initial Pendulum Position.  Hit Any Key to Begin." );
		_outtext( txt );

		draw_pend( th_old, 0, anima, pend, 'r' );
		if( save_flag )
		...
		while( t <= p.tmax )
		{
			...
			if( fabs( th_new ) > PI )
				th_new -= 2.0000 * PI * fabs( th_new ) / th_new;

			t += dt;

			if( show_graphics )
			{
				draw_pend( th_new, th_old, anima, pend, 'r' );
	...
	else
	{
		...
		printf( "\n\n\n\n\n\n Can't Open %s.  Hit any key.", direc );
		exit( 11 );
	}
