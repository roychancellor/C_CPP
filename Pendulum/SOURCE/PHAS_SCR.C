...
*    VERSION:              1.00                                           *
*                                                                         *
*    This program sets up the graphs for pendulum animation.              *
*                                                                         *
**************************************************************************/
...
/* LINEAR-LOGARITHMIC */
	...
	{
		xrge = ( xy.hx ) - ( xy.lx );
		yrge = log10( xy.hy ) - log10( xy.ly );
		xlo = ( xy.lx ) - 0.2 * xrge;
		ylo = log10( xy.ly ) - 0.3 * yrge;
		xhi = ( xy.hx ) + 0.2 * xrge;
		yhi = log10( xy.hy ) + 0.2 * yrge;

		_setwindow( TRUE, xlo, ylo, xhi, yhi );
		_setcolor( xy.back_color );
		_rectangle_w( _GFILLINTERIOR, xlo, ylo, xhi, yhi );
		_setcolor( xy.border_color );
		_rectangle_w( _GBORDER, xy.lx, log10( xy.ly ), xy.hx, log10( xy.hy ) );
	}
/* LOGARITHMIC-LINEAR */
	else if( (!strcmp( xy.type_x, "log\n" )) &&
													(!strcmp( xy.type_y, "lin\n" )) )
	{
		yrge = ( xy.hy ) - ( xy.ly );
		xrge = log10( xy.hx ) - log10( xy.lx );
		ylo = ( xy.ly ) - 0.3 * yrge;
		xlo = log10( xy.lx ) - 0.2 * xrge;
		yhi = ( xy.hy ) + 0.2 * yrge;
		xhi = log10( xy.hx ) + 0.2 * xrge;

		_setwindow( TRUE, xlo, ylo, xhi, yhi );
		_setcolor( xy.back_color );
		_rectangle_w( _GFILLINTERIOR, xlo, ylo, xhi, yhi );
		_setcolor( xy.border_color );
		_rectangle_w( _GBORDER, log10( xy.lx ), xy.ly, log10( xy.hx ), xy.hy );
	}
/* LOGARITHMIC-LOGARITHMIC */
													...
													(!strcmp( xy.type_y, "log\n" )) )
	{
		...
		_rectangle_w( _GFILLINTERIOR, xlo, ylo, xhi, yhi );
		_setcolor( xy.border_color );
		_rectangle_w( _GBORDER, log10( xy.lx ), log10( xy.ly ),
														  log10( xy.hx ), log10( xy.hy ) );
	}
...
void draw_graph_axes( struct axis_data xy, int do_xax, int do_yax )
{
	...
	if( do_xax == LOG )
	{
		...
		xhi = log10( xy.hx );
		xlo = log10( xy.lx );
		num_decades_x = (int)floor( xrge );
		num_times_x = xy.ntx * num_decades_x;
	}
	if( do_yax == LIN )
	{
	...
	if( do_yax == LOG )
	{
		...
		yhi = log10( xy.hy );
		ylo = log10( xy.ly );
		num_decades_y = (int)floor( yrge );
		num_times_y = xy.nty * num_decades_y;
	}
